...
const copy = {
  ...
  zh: {
    ...
    generatedByAI: "AI 实时生成",
    generatedLocally: "本地生成",
    modalSource: "查看来源",
...
function promptGenerateEndpoint() {
  return metaContent("prompt-generate-endpoint") || "/api/generate-prompt";
}
...
async function verifyTurnstileToken() {
  ...
  turnstileVerifyPromise = fetch(turnstileVerifyEndpoint(), {
    ...
    .then(async (response) => {
      ...
      state.turnstileAccessToken = result.accessToken || "";
...
function setGenerating(isGenerating) {
  state.generatingPrompt = isGenerating;
  const label = document.querySelector("#generateLabel");
  if (label) label.textContent = isGenerating ? t("generatingLabel") : t("generateLabel");
  const button = els.builderForm.querySelector(".generate-button");
...
async function requestGeneratedPrompt({ subject, model, ratio }) {
  const response = await fetch(promptGenerateEndpoint(), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      subject,
      ...
      accessToken: state.turnstileAccessToken,
    }),
  });
  const result = await response.json().catch(() => ({}));
...
async function generatePrompt() {
  ...
  const localPrompt = buildLocalPrompt({ subject, model, ratio });

  setGenerating(true);
  try {
    const prompt = await requestGeneratedPrompt({ subject, model, ratio });
    renderGeneratedPrompt(prompt, t("generatedByAI"));
  } catch {
    renderGeneratedPrompt(localPrompt, t("generatedLocally"));
